...
The [DEBUG](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L19)
...
!!! note
    ...
    check the official [documentation](https://help.disqus.com/customer/portal/articles/236206).
...
The [HOST](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L22)
and [PORT](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L23)
...
only need to uncomment the [HOST](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L22)
variable in order to listen in all the net interfaces.

## Securing the server

PYBOSSA uses the [Flask Sessions](http://flask.pocoo.org/docs/quickstart/#sessions) feature that
...
and
[SECRET_KEY](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L26).
...
modify the [ITSDANGEROUSKEY](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L35).
...
**pybossa/pybossa/static/img**. If the logo name is, **my_brand.png**
the [LOGO](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl#L40)
...
**settings_local.py**. For example, to remove French you can add this
...
named results.html to the home directory in the templates folder and
...
variables in the **settings_local.py** file:
...
folder: templates/help/ a file named **index.html** and write your
...
key and secret** into the next variables and uncomment them:
...
the **app ID/API Key and secret** into the next variables and uncomment them:
...
**Client ID and secret** into the next variables and uncomment them:
``` python
...
[settings_local.py.tmpl](https://github.com/Scifabric/pybossa/blob/master/settings_local.py.tmpl)
...
fit your institutional needs. In the case that you do not want to use
the default one, please, create a **_cookies_policy.html** file in the
**custom** folder. You can re-use the template
**help/_cookies_policy.html** and adapt it (it is located in the
...
file named **_privacy_policy.html** file in the **custom** folder.
...
!!! warning
    Be very careful. This is your responsibility, and it's not
...
the settings_local.py file the following config variables:
...
**TEMPLATE_TASKS** in settings_local.py:
...
the **Consumer key and secret** into the next variables and uncoment them:
``` python
...
*disqus_sso*. You can use it like this:
...
tasks or task_runs. This is a bad thing for the exporter, as it will
...
task_presenter when publishing a project. If you need this, just add
